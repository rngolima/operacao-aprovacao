...
## 🗺️ Mapa da Sprint 0: Os 5 Passos Fundamentais

- [x] **PASSO 1:** O Manifesto do Projeto — pom.xml, Apache Maven e Gestão de Dependências.
- [x] **PASSO 2:** O Ponto de Ignição — OperacaoAprovacaoApplication.java e o Ciclo de Vida do Spring Boot.
- [x] **PASSO 3:** O Modelo Base e Envelope — BaseEntity.java e ApiResponse.java.
- [x] **PASSO 4:** A Fundação dos Dados — V1__initial_schema.sql, PostgreSQL e Flyway.
- [ ] **PASSO 5:** Camada Web e Testes — HealthController.java e OperacaoAprovacaoApplicationTests.java.
...
O pom.xml é a certidão de nascimento de qualquer aplicação Java corporativa. Ele gerencia o ciclo de vida do build (compile, 	est, package), declara a versão da linguagem (Java 21 LTS) e orquestra o download automatizado de bibliotecas a partir do repositório central do Maven.
...
1. spring-boot-starter-web: Fornece o Spring MVC e embute o servidor Apache Tomcat na porta 8080.
2. spring-boot-starter-data-jpa: Traz o Hibernate para abstração de persistência relacional.
3. spring-boot-starter-security: Fornece a infraestrutura de filtros para segurança e controle de acesso.
4. postgresql: Driver JDBC de alto desempenho para comunicação com o banco de produção (Supabase).
5. lyway-core: Ferramenta para versionamento e migração idempotente de schema SQL.
6. springdoc-openapi-starter-webmvc-ui: Gera documentação viva e interativa via Swagger UI em /swagger-ui.html.

### 🎯 Simulação de Entrevista Técnica (Passo 1):
- **Pergunta do Tech Lead:** *"Vi no seu pom.xml que você usou o lyway-core junto com o Spring Data JPA. Por que você não deixou o próprio Hibernate criar as tabelas automaticamente usando spring.jpa.hibernate.ddl-auto=update?"*
- **Resposta Técnica Modelo:**  
  ...
  > Adotamos o **Flyway** porque ele atua como o versionador do banco de dados (o 'Git das tabelas'), executando scripts SQL imutáveis (V1, V2, etc.) validados por checksum na tabela lyway_schema_history. Isso garante **idempotência**, **rastreabilidade estrita** e **reprodutibilidade idêntica** em todos os ambientes da nossa esteira de CI/CD."*
...
- **Pergunta do Tech Lead:** *"Se eu criar uma classe @RestController nova dentro de um pacote fora de com.operacaoaprovacao.api (por exemplo, em com.meuoutroprojeto.controllers), o Spring Boot vai conseguir encontrar e subir esse controller? Por quê?"*
- **Resposta Técnica Modelo:**  
  ...
  > Como o controller está em um pacote irmão ou fora da árvore hierárquica, o radar do Spring não o enxerga, a menos que declaremos explicitamente esse pacote externo no parâmetro asePackages do @ComponentScan."*
...
- **@MappedSuperclass:** Avisa ao JPA que esta classe serve exclusivamente para herança de colunas, instruindo o banco a **não criar uma tabela física** chamada ase_entity.
...
- **Pergunta do Tech Lead:** *"Por que você anotou a classe BaseEntity com @MappedSuperclass em vez de anotar com @Entity?"*
- **Resposta Técnica Modelo:**  
  ...
  > Com @MappedSuperclass, nenhuma tabela é gerada para a classe abstrata; suas colunas e restrições são incorporadas diretamente nas tabelas físicas das entidades filhas (	b_usuario, 	b_concurso, etc.)."*
...
- **Normalização (3ª Forma Normal):** A entidade 	b_concurso não armazena strings literais da banca ("Cebraspe"). Ela aponta para anca_id. Se a banca atualizar seus dados corporativos, a alteração ocorre em uma única linha, mantendo integridade e consistência relacional.
